# R-S2 round 10 — QA-A1 bootstrap E2E (WP_INSTALLING on poll)
$ErrorActionPreference = 'Stop'
$Repo = (Resolve-Path (Join-Path $PSScriptRoot '..\..')).Path
$Evidence = Join-Path $Repo 'docs\qa-evidence\approach-b-retest-2026-05\R-S2-round10'
...
$wpInstalling = (docker exec $A1 bash -lc "grep -c 'WP_INSTALLING' /var/www/html/wp-content/plugins/museder-restoreone/includes/class-restore-bootstrap.php").Trim()
if ([int]$wpInstalling -lt 1) { throw 'Round 10 WP_INSTALLING fix not deployed' }
Write-Host "deployed WP_INSTALLING refs: $wpInstalling"
...
Write-Host '[R-S2 R10] POST start...'
...
curl.exe -s -D $pollHeadersFile -o (Join-Path $Evidence 'poll-0-body.html') -w '%{http_code}' --max-time 300 $pollUrl | Out-Null
...
if ($isInstallRedirect) { Write-Host 'WARN: poll-0 redirects to install.php' }
if (-not $hasBootstrapHdr) { Write-Host 'WARN: poll-0 missing X-Museder-Restoreone-Bootstrap header' }

$finished = $false
$pollStart = 1
for ($i = $pollStart; $i -lt 240; $i++) {
    ...
    $isBootstrap = $pollBody -match 'Restore Bootstrap|museder_bootstrap|Museder RestoreOne'
...
docker logs $A1 2>&1 | Select-String -Pattern 'Fatal' | Select-Object -Last 5 | Out-String | Set-Content (Join-Path $Evidence 'apache-fatals-tail.txt') -Encoding utf8
