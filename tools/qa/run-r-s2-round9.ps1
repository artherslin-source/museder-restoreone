# R-S2 round 9 — QA-A1 bootstrap E2E (two-phase request: POST files / poll DB)
$ErrorActionPreference = 'Stop'
$Repo = (Resolve-Path (Join-Path $PSScriptRoot '..\..')).Path
$Evidence = Join-Path $Repo 'docs\qa-evidence\approach-b-retest-2026-05\R-S2-round9'
...
docker exec $A1 bash -lc "grep -c bootstrap_stubs_are_active /var/www/html/wp-content/plugins/museder-restoreone/includes/class-restore-bootstrap.php"
if ($LASTEXITCODE -ne 0) { throw 'Round 9 bootstrap not deployed' }
...
for ($i = 0; $i -lt 240; $i++) {
    ...
    $isBootstrap = $pollBody -match 'Restore Bootstrap|museder_bootstrap'
...
docker logs $A1 2>&1 | Select-String -Pattern 'Fatal' | Select-Object -Last 5 | Set-Content (Join-Path $Evidence 'apache-fatals-tail.txt') -Encoding utf8
