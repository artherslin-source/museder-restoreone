...
class Backup_Lite_Restore_Service {
    ...
    protected static function summarise_database_structure( array $metadata ) {
        ...
        foreach ( $tables as $table ) {
            ...
            // Introspection query used for restore validation summary.
            // $name comes from the archive metadata generated by this plugin (admin-provided backup), not from untrusted user input.
    ...
    protected static function run_search_replace( $pairs ) {
        ...
        foreach ( $tables as $table ) {
            ...
            // Introspection required for search/replace after restore.
            ...
            // Full table scan is required to reliably update serialized/text fields during restore migration.
            // Table identifier cannot be prepared; validated via backup_lite_validate_wp_table_name().
            // phpcs:disable WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
    ...
    protected static function post_restore_cleanup() {
        ...
        // Clear transients
        // Cleanup query used after restore to remove stale transients.
        // $wpdb->options is a core WP table; patterns are hardcoded.
        // phpcs:disable WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
