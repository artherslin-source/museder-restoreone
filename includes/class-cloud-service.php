...
class Museder_Cloud_Service {
    ...
    protected static function upload_to_s3( string $file_path, array $settings, array $context = [] ): array {
        ...
        // Parse endpoint URL
        $endpoint_url = wp_parse_url( $endpoint );
        ...
        if ( $status_code >= 400 ) {
            ...
            return [
                'status'  => 'error',
                /* translators: 1: HTTP status code, 2: Error message excerpt. */
                'message' => sprintf( __( 'HTTP %1$d: %2$s', 'museder-restoreone' ), $status_code, $error_excerpt ),
            ];
        }
