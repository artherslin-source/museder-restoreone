...
= 2.7.276 =
Fixes Exit Safe Mode failing with a generic error after restore when DB import breaks admin-ajax session or overwrites the restore HMAC secret in uploads. This release also makes Step 2 restore scope/pre-backup choices visible and traceable, and ensures cancelled/failed Step 3 jobs update restore history/final-status instead of staying as running. Strongly recommended if restore completes but Exit Safe Mode shows “An unexpected error occurred,” or if Step 3 appears stuck after cancellation.
