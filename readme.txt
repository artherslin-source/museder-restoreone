...
= 2.7.96 =
* Fix: Prevent backups from being marked completed before packing finishes; adds finalize safety checks to avoid incomplete archives.
* Fix: Fail the job if ZipArchive cannot add a file (instead of silently producing an incomplete ZIP); see logs for the first failing path.

= 2.7.97 =
* Fix: Improve compatibility on hosts using symlinked paths/open_basedir by avoiding realpath() during manifest building; ensures files can be found and packed correctly.
...
= 2.7.102 =
* Fix: Prevent “fake success” backups by validating manifest integrity and added files/bytes at finalize-after-close; incomplete archives now fail with clear diagnostics.
