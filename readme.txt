...
* Restore Step 2: show a live "Will execute" summary and log a structured restore enqueue snapshot so visible radio state, submitted FormData, backend parsed options, normalized options, and job meta can be compared.
* Restore Step 2: populated destinations now explicitly explain that the pre-restore snapshot is required and cannot be disabled.
* Restore Step 3: cancelled/failed terminal jobs now repair restore history and final-status payloads so UI polling stops on `cancelled` or `failed` instead of leaving a stale `running` history row.
* Restore DB stage: add structured telemetry for process slice stage entry/exit, `restore-db` file/open/import checkpoints, and explicit stale warnings when `restore-db` is stuck with `db_offset=0` so shared-host blockers are diagnosable from logs.
* Restore DB stage: NDJSON import now runs in resumable time slices with persisted byte-offset checkpoints so admin-ajax/cron ticks can advance large database imports on shared hosts instead of restarting from `db_offset=0` after each request timeout.
* Restore liveness: status/final-status readers now self-heal orphaned running jobs by re-seeding missing `museder_restoreone_restore_service_process_job` cron events and reasserting the active job pointer when a non-terminal restore stage is detected.
...
* Restore UI: preserve the completed job's restore token until Safe Mode exit succeeds, and make both completion overlay and Safe Mode notice buttons send the same job-bound token/grant payload instead of relying on nonce-only requests.
* Restore UI: Step 3 now hard-blocks full-site execution when preflight says overwrite is required but not enabled, so the button state matches server preconditions instead of failing with late 400 errors.
* Restore UI: auth-check interruptions now use a restore-specific login redirect (`wp-login.php?redirect_to=...restore...`) to keep reauth return paths pinned to Restore page resume flow instead of drifting to host-specific admin routes.
* Restore UI: harden auth-check race handling by removing late-injected WordPress interim-login/thickbox modals while restore is running or final success is locked.
...
= 2.7.276 =
Fixes Exit Safe Mode failing with a generic error after restore when DB import breaks admin-ajax session or overwrites the restore HMAC secret in uploads. This release also makes Step 2 restore scope/pre-backup choices visible and traceable, ensures cancelled/failed Step 3 jobs update restore history/final-status instead of staying as running, adds restore-db telemetry/stale diagnostics (`db_offset=0` blocker signals) for shared-host database-import troubleshooting, resumes NDJSON database import across admin-ajax/cron slices with persisted byte-offset checkpoints, and adds liveness self-heal that re-schedules missing restore process cron events from status/final-status readers. Strongly recommended if restore completes but Exit Safe Mode shows “An unexpected error occurred,” if Step 3 appears stuck after cancellation, or if Full restore stalls at “Preparing database import…” / progress 80%.
