...
## 6) 新增強制驗收：Docker 大/小站備份還原測試

依任務要求，正式提交前新增硬性 Gate：

### 6.1 測試範圍
- 在 Docker 建立 WordPress 測試站。
- 安裝待提交外掛 zip。
- 建立兩種資料規模：
  - 小站 `< 2GB`
  - 大站 `> 2GB`
...
### 6.3 提交門檻
- 兩種情境皆通過，版本才可視為「正式可提交」。
- 任一失敗即 No-Go，需修正後重跑。

### 6.4 報告要求
- 完成後提交任務報告（測試環境、資料組成、步驟、耗時、結果、異常與修正、最終是否可提審）。
...
只要把「掃描 → 修正 → 證據 → 打包 gate → Docker 大/小站實測驗收」固定化，就能大幅降低下一輪退件機率，同時保住外掛功能目標。
