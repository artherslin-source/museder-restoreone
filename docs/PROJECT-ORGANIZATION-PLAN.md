...
## 9) 新增驗收關卡：Docker 實站備份/還原測試（提交前必要）

依最新任務要求，加入**正式提交前的強制驗收**：

### 必測環境
1. 以 Docker 建立乾淨 WordPress 測試站。  
2. 安裝待提交版本外掛（release zip）。

### 必測情境
1. **小站測試（<2GB）**
   - 含常見內容：頁面、文章、圖片媒體
   - 含常見元件：多個外掛、佈景
   - 驗證：可成功備份、可成功還原、前後台功能可正常開啟

2. **大站測試（>2GB）**
   - 含大量媒體與常見實務內容（外掛/佈景/文章/頁面/圖片）
   - 驗證：可完成備份、可完成還原（可接受分段/續跑流程）
   - 驗證還原後：登入管理後台、前台可瀏覽、媒體可讀取

### 驗收標準（Go / No-Go）
- 兩種情境都完成「備份 + 還原」且結果可用，才視為**可提交 WordPress 審查**版本。
- 任一情境失敗，該版本不得提交，需修正後重測。

### 交付物
- 測試完成後需提交任務報告（測試資料、步驟、結果、失敗點與修正、最終結論）。

---

## 10) 這份方案的定位

這是一份**零風險優先**的整理方案：先清楚、可維護、可追蹤，再進入功能修正與過審衝刺；且現在已加入 Docker 大/小站備份還原驗收關卡，作為正式提審前必要條件。
